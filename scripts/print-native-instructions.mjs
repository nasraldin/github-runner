...
if (!requestedProject || !requestedPool) {
  fail(
    "Usage: node scripts/print-native-instructions.mjs <project-id> <pool-id>",
  );
}
...
if (runnerPackage.os === "macos" || runnerPackage.os === "linux") {
  ...
  if (runnerPackage.sha256) {
    console.log(
      `echo "${runnerPackage.sha256}  ${runnerPackage.archive}" | shasum -a 256 -c`,
    );
  }
  console.log(`tar xzf ./${runnerPackage.archive}`);
  console.log(
    `./config.sh --url ${repoUrl} --token "$GITHUB_RUNNER_REGISTRATION_TOKEN" --name "${unixRunnerName}" --labels "${labels}" --work "${pool.runnerWorkdir ?? config.defaults.runnerWorkdir}" --unattended --replace`,
  );
  console.log(`./run.sh`);
  process.exit(0);
}
...
if (runnerPackage.os === "windows") {
  ...
  console.log(
    `Invoke-WebRequest -Uri ${runnerPackage.url} -OutFile ${runnerPackage.archive}`,
  );
  if (runnerPackage.sha256) {
    console.log(
      `if((Get-FileHash -Path ${runnerPackage.archive} -Algorithm SHA256).Hash.ToUpper() -ne '${runnerPackage.sha256}'.ToUpper()){ throw 'Computed checksum did not match' }`,
    );
  }
  console.log(`Add-Type -AssemblyName System.IO.Compression.FileSystem`);
  console.log(
    `[System.IO.Compression.ZipFile]::ExtractToDirectory("${archivePath}", "$PWD")`,
  );
  console.log(
    String.raw`.\config.cmd` +
      ` --url ${repoUrl} --token "$env:GITHUB_RUNNER_REGISTRATION_TOKEN" --name "${windowsRunnerName}" --labels "${labels}" --work "${pool.runnerWorkdir ?? config.defaults.runnerWorkdir}" --unattended --replace`,
  );
